...
public interface CazeInstanceRepository extends JpaRepository<CazeInstance, Long> {
    ...
    @Query(value = "select distinct caze_instance from CazeInstance caze_instance left join fetch caze_instance.relatedCazes",
    ...
    @Query("select caze_instance from CazeInstance caze_instance left join fetch caze_instance.relatedCazes where caze_instance.id =:id")
